...
async function autoSeedMenu() {
  try {
    // Check if menu has the correct schema (has numeric id field)
    const sample = await MenuItem.findOne({ id: { $exists: true } });
    if (sample) {
      const count = await MenuItem.countDocuments();
      console.log(`[MenuSeed] Menu already has ${count} items with correct schema — skipping seed.`);
      return;
    }

    // Drop old menu data that lacks id/desc fields
